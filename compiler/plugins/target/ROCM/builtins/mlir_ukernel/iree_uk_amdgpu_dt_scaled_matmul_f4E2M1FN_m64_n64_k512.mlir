...
!return_ty = tensor<1x1x4x4x4x16x4xf32>

!acc_ty = vector<4x1x4x4x1xf32>

!reduce_ty = vector<4x1x4x1xf32>
!store_ty = vector<1x1x1x4x1x1x4xf32>
!tensor_store_ty = tensor<1x1x1x4x1x1x4xf32>
...
util.func @dt_scaled_matmul_f4f4f32_m64_n64_k512(
  ...
  scf.forall (%base_id) in (512) {
    ...
    scf.if %cmp {
      ...
      scf.for %i = %c0 to %k step %c1 {
        ...
        amdgpu.gather_to_lds %rhs_scale[%c0, %i, %ids#0, %ids#1, %ids#2, %c0], %rhs_scale_shared[%buffer_num, %ids#0, %ids#1, %ids#2, %c0]
  ...
  %1 = scf.forall (%id) in (256) shared_outs(%out = %0) -> !return_ty {
    ...
    %loop = scf.for %i = %c0 to %k step %c1 iter_args(%iter = %init) -> !acc_ty {
      ...
      %rhs_byte_vec = vector.transfer_read %rhs_shared_base[%buffer_num, %ids#0, %c0, %ids#1, %ids#2, %c0],
      ...
      %rhs_scale_byte_vec = vector.transfer_read %rhs_scale_shared[%buffer_num, %ids#0, %ids#1, %ids#2, %c0],
    ...
    %to_tensor = vector.transfer_write %t, %empty[%c0, %c0, %c0, %c0, %c0, %c0, %c0]
      {in_bounds = [true, true, true, true, true, true, true]} : !store_ty, !tensor_store_ty

    scf.forall.in_parallel {
      tensor.parallel_insert_slice %to_tensor into %out[%c0, %c0, %ids#0, %c0, %ids#1, %ids#2, %c0] [1, 1, 1, 4, 1, 1, 4] [1, 1, 1, 1, 1, 1, 1]
