...
!rhs_scale_copy_vec_ty = vector<2xi8>
...
util.func @dt_scaled_matmul_f4f4f32_m32_n64_k2048(
    %lhs_base: !lhs_ty,
    %rhs_base: !rhs_ty,
  ...
  scf.forall (%base_id) in (512) {
    ...
    scf.for %i = %c0 to %c2 step %c1 {
      ...
      amdgpu.gather_to_lds %rhs[%c0, %c0, %c0, %rhs_outer, %ids#1, %ids#2, %ids#3, %c0], %rhs_shared_base[%c0, %rhs_outer, %ids#1, %ids#2, %ids#3, %c0] {aux = 3 : i32}
    ...
    %scale_id_inner = arith.muli %scale_ids#3, %c2 : index
    ...
    amdgpu.gather_to_lds %rhs_scale[%c0, %c0, %scale_ids#0, %scale_ids#1, %scale_ids#2, %scale_id_inner], %rhs_scale_shared[%c0, %scale_ids#0, %scale_ids#1, %scale_ids#2, %scale_id_inner] {aux = 3 : i32}
    ...
    %loop = scf.for %i = %c1 to %k step %c1 iter_args(%iter = %init) -> !acc_ty {
      ...
      scf.for %j = %c0 to %c2 step %c1 {
        ...
        amdgpu.gather_to_lds %lhs[%c0, %i, %c0, %j, %subgroup_ids#0, %subgroup_ids#1, %ids#1, %ids#2, %c0], %lhs_shared_base[%buffer_num, %j, %subgroup_ids#0, %subgroup_ids#1, %ids#1, %ids#2, %c0]
          : !lhs_copy_vec_ty, !lhs_buffer_ty, !lhs_shared_ty
        amdgpu.gather_to_lds %rhs[%c0, %i, %c0, %rhs_outer, %subgroup_ids#1, %ids#1, %ids#2, %c0], %rhs_shared_base[%buffer_num, %rhs_outer, %subgroup_ids#1, %ids#1, %ids#2, %c0] {aux = 3 : i32}
          : !rhs_copy_vec_ty, !rhs_buffer_ty, !rhs_shared_ty
      }
      ...
      amdgpu.gather_to_lds %lhs_scale[%c0, %i, %scale_id_outers#0, %scale_id_outers#1, %scale_ids#1, %scale_ids#2, %scale_id_inner], %lhs_scale_shared[%buffer_num, %scale_id_outers#0, %scale_id_outers#1, %scale_ids#1, %scale_ids#2, %scale_id_inner]
        : !lhs_scale_copy_vec_ty, !lhs_scale_buffer_ty, !lhs_scale_shared_ty
      amdgpu.gather_to_lds %rhs_scale[%c0, %i, %scale_ids#0, %scale_ids#1, %scale_ids#2, %scale_id_inner], %rhs_scale_shared[%buffer_num, %scale_ids#0, %scale_ids#1, %scale_ids#2, %scale_id_inner] {aux = 3 : i32}
        : !rhs_scale_copy_vec_ty, !rhs_scale_buffer_ty, !rhs_scale_shared_ty
