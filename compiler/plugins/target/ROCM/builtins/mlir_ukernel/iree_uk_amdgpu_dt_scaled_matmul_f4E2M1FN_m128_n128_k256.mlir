...
!lhs_ty = tensor<1x?x1x8x2x4x16x32x!lhs>
!rhs_ty = tensor<1x?x1x4x2x2x4x16x32x!rhs>

!lhs_byte_ty = tensor<1x?x1x8x2x4x16x16xi8>
!rhs_byte_ty = tensor<1x?x1x4x2x2x4x16x16xi8>

!lhs_buffer_ty = memref<1x?x1x8x2x4x16x16xi8, strided<[?, 16384, 16384, 2048, 1024, 256, 16, 1], offset: ?>, #amdgpu.address_space<fat_raw_buffer>>
!rhs_buffer_ty = memref<1x?x1x4x2x2x4x16x16xi8, strided<[?, 16384, 16384, 4096, 2048, 1024, 256, 16, 1], offset: ?>, #amdgpu.address_space<fat_raw_buffer>>

// 2 = double buffer
// 128 = m tile
// 128 = n tile
// 8 = k split factor
// 32 = 256 / 8 = k tile / k split
!lhs_shared_ty = memref<2x8x2x4x16x16xi8, #gpu.address_space<workgroup>>
!rhs_shared_ty = memref<2x4x2x2x4x16x16xi8, #gpu.address_space<workgroup>>
...
!rhs_byte_vec_ty = vector<2x1x2x16xi8>
!rhs_vec_ty = vector<2x1x2x32xf4E2M1FN>

// Scales

!lhs_scale_ty = tensor<1x?x8x4x16x2x!scale_ty>
!rhs_scale_ty = tensor<1x?x4x2x4x16x2x!scale_ty>

!lhs_scale_byte_ty = tensor<1x?x8x4x16x2xi8>
!rhs_scale_byte_ty = tensor<1x?x4x2x4x16x2xi8>

!lhs_scale_buffer_ty = memref<1x?x8x4x16x2xi8, strided<[?, 1024, 128, 32, 2, 1], offset: ?>, #amdgpu.address_space<fat_raw_buffer>>
!rhs_scale_buffer_ty = memref<1x?x4x2x4x16x2xi8, strided<[?, 1024, 256, 128, 32, 2, 1], offset: ?>, #amdgpu.address_space<fat_raw_buffer>>
...
// 128 = n tile
// 2 = k split factor
// 2 = k tile / k block (256/128)
!lhs_scale_shared_ty = memref<2x8x4x16x2xi8, #gpu.address_space<workgroup>>
!rhs_scale_shared_ty = memref<2x4x2x4x16x2xi8, #gpu.address_space<workgroup>>

!lhs_scale_byte_vec_ty = vector<8x1x2x1xi8>
!lhs_scale_vec_ty = vector<8x1x2x1xf8E8M0FNU>

!rhs_scale_byte_vec_ty = vector<2x1x2x1xi8>
!rhs_scale_vec_ty = vector<2x1x2x1xf8E8M0FNU>

!res_ty = tensor<8x16x8x16xf32>
!return_ty = tensor<1x1x4x8x2x4x16x4xf32>

!acc_ty = vector<8x2x2x4x1xf32>

!reduce_ty = vector<8x2x4x1xf32>
!store_ty = vector<1x1x1x8x2x1x1x4xf32>
!tensor_store_ty = tensor<1x1x1x8x2x1x1x4xf32>
...
util.func @dt_scaled_matmul_f4f4f32_m128_n128_k256(
    %lhs_base: !lhs_ty,
    %rhs_base: !rhs_ty,
  ...
  scf.forall (%base_id) in (512) {
    ...
    scf.if %cmp {
      ...
      %ls_ids:4 = affine.delinearize_index %id into (4, 8, 4, 2) : index, index, index, index
      %rs_ids:5 = affine.delinearize_index %id into (4, 4, 2, 4, 2) : index, index, index, index, index
      %ls_inner = arith.muli %ls_ids#3, %c8 : index
      %rs_inner = arith.muli %rs_ids#4, %c8 : index
      scf.for %i = %c0 to %k step %c1 {
        %buffer_num = arith.andi %i, %c1 : index
        // Copy inputs.
        scf.for %j = %c0 to %c4 step %c1 {
          %outer_base = arith.muli %j, %c2 : index
          %outer = arith.addi %ids#0, %outer_base : index
          amdgpu.gather_to_lds %lhs[%c0, %i, %c0, %outer, %ids#1, %ids#2, %ids#3, %c0], %lhs_shared_base[%buffer_num, %outer, %ids#1, %ids#2, %ids#3, %c0]
        ...
        // Copy scales.
        scf.for %j = %c0 to %c4 step %c1 {
          amdgpu.gather_to_lds %rhs[%c0, %i, %c0, %j, %ids#0, %ids#1, %ids#2, %ids#3, %c0], %rhs_shared_base[%buffer_num, %j, %ids#0, %ids#1, %ids#2, %ids#3, %c0]
  ...
  %1 = scf.forall (%id) in (256) shared_outs(%out = %0) -> !return_ty {
    ...
    %loop = scf.for %i = %c0 to %k step %c1 iter_args(%iter = %init) -> !acc_ty {
      ...
      %lhs_byte_vec = vector.transfer_read %lhs_shared_base[%buffer_num, %c0, %c0, %ids#1, %ids#2, %c0],
        %cst_lhs {in_bounds = [true, true, true, true, true, true]} : !lhs_shared_ty, vector<1x8x2x1x1x16xi8>
      %lhs_byte_vec_t = vector.shape_cast %lhs_byte_vec : vector<1x8x2x1x1x16xi8> to !lhs_byte_vec_ty
      ...
      %lhs_scale_byte_vec = vector.transfer_read %lhs_scale_shared[%buffer_num, %c0, %ids#1, %ids#2, %c0],
        %cst_scale {in_bounds = [true, true, true, true, true]} : !lhs_scale_shared_ty, vector<1x8x1x1x2xi8>
      %lhs_scale_byte_vec_t = vector.shape_cast %lhs_scale_byte_vec : vector<1x8x1x1x2xi8> to !lhs_scale_byte_vec_ty
      ...
      %rhs_byte_vec = vector.transfer_read %rhs_shared_base[%buffer_num, %ids#0, %c0, %c0, %ids#1, %ids#2, %c0],
        %cst_rhs {in_bounds = [true, true, true, true, true, true, true]} : !rhs_shared_ty, vector<1x1x2x2x1x1x16xi8>
      %rhs_byte_vec_t = vector.shape_cast %rhs_byte_vec : vector<1x1x2x2x1x1x16xi8> to !rhs_byte_vec_ty
      ...
      %rhs_scale_byte_vec = vector.transfer_read %rhs_scale_shared[%buffer_num, %ids#0, %c0, %ids#1, %ids#2, %c0],
        %cst_scale {in_bounds = [true, true, true, true, true, true]} : !rhs_scale_shared_ty, vector<1x1x2x1x1x2xi8>
      %rhs_scale_byte_vec_t = vector.shape_cast %rhs_scale_byte_vec : vector<1x1x2x1x1x2xi8> to !rhs_scale_byte_vec_ty
    ...
    scf.forall.in_parallel {
      tensor.parallel_insert_slice %to_tensor into %out[%c0, %c0, %ids#0, %c0, %c0, %ids#1, %ids#2, %c0] [1, 1, 1, 8, 2, 1, 1, 4] [1, 1, 1, 1, 1, 1, 1, 1]
