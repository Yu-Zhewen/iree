...
chooseDataTiledMMAAttr(TypeRange eTypes, TargetAttr target,
  ...
  IREE::Codegen::UKernelProviderInterface ukernelProvider =
      getUKernelProviderFromTarget(config);
  if (ukernelProvider) {
    IREE::GPU::DataTiledMMAAttr mma =
        dyn_cast_if_present<IREE::GPU::DataTiledMMAAttr>(
            ukernelProvider.getDataLayoutForUKernel(encoding, config));
    if (mma) {
      return mma;
    }
  }
