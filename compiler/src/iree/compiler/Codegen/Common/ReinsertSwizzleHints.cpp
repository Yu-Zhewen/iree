...
lookupSwizzleAttr(
  ...
  auto swizzle =
      allocOp->getAttrOfType<IREE::Codegen::SwizzleAttrInterface>(
...
static Value insertSwizzleHint(IRRewriter &rewriter, Location loc,
                               Value source,
  ...
  if (sourceType.getRank() > 1) {
    ...
    hintInput =
        memref::CollapseShapeOp::create(rewriter, loc, source, reassoc);
  }
