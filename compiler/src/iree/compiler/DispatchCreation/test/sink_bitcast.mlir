...
// CHECK-SAME: %[[ARG0:[A-Za-z0-9]+]]: tensor<?xf4E2M1FN>
// CHECK-SAME: %[[D0:[A-Za-z0-9]+]]: index
// CHECK-SAME: %[[D1:[A-Za-z0-9]+]]: index
func.func @propagateBitCastThroughExpandShape(%arg0: tensor<?xf4E2M1FN>, %d0: index, %d1: index) -> tensor<4x?x512x16xi8> {
  // CHECK: %[[EXPAND:.+]] = tensor.expand_shape %[[ARG0]] {{\[\[}}0, 1, 2, 3]]
  // CHECK-SAME: output_shape [4, %[[D1]], 512, 32]
  // CHECK-SAME: tensor<?xf4E2M1FN> into tensor<4x?x512x32xf4E2M1FN>
  %0 = iree_tensor_ext.bitcast %arg0 : tensor<?xf4E2M1FN>{%d0} -> tensor<?xi8>{%d0}
  %1 = tensor.expand_shape %0 [[0, 1, 2, 3]] output_shape [4, %d1, 512, 16]
      : tensor<?xi8> into tensor<4x?x512x16xi8>
